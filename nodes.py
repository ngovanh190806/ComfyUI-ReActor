...
def get_restorers():
    ...
    # --- Logic tự động tải RestoreFormer++ (Tham khảo tungdop2) ---
    rf_name = "RestoreFormer++.pth" # Đổi đuôi ckpt thành pth để UI tự nhận diện
    rf_path = os.path.join(dir_facerestore_models, rf_name)
    if not os.path.exists(rf_path):
        # Tải từ HuggingFace của tungdop2
        download("https://huggingface.co/tungdop2/FaceRestorer/resolve/main/RestoreFormer++.ckpt", rf_path, rf_name)
    # -------------------------------------------------------------
...
class reactor:
    ...
    def restore_face(
        ...
        if face_restore_model != "none" and not model_management.processing_interrupted():
            ...
            # --- Khởi tạo đối tượng mô hình (Logic từ code mẫu) ---
            if "restoreformer" in face_restore_model.lower():
                from scripts.r_archs.arch_RestoreFormer import VQVAEGANMultiHeadTransformer
                
                facerestore_model_obj = VQVAEGANMultiHeadTransformer(
                    n_embed=1024, embed_dim=256, ch=64, out_ch=3, ch_mult=(1, 2, 2, 4, 4, 8),
                    ...
                    fix_decoder=False, fix_codebook=True, fix_encoder=False, head_size=4, ex_multi_scale_num=1
                ).to(device)
                
                checkpoint = torch.load(model_path, map_location='cpu')
                # Lấy state_dict từ checkpoint
                state_dict = checkpoint.get('params_ema', checkpoint.get('state_dict', checkpoint))
                
                # --- SỬA LỖI: Loại bỏ tiền tố 'vqvae.' ---
                new_state_dict = {}
                for k, v in state_dict.items():
                    # Nếu khóa bắt đầu bằng 'vqvae.', bỏ phần đó đi
                    new_key = k.replace("vqvae.", "")
                    new_state_dict[new_key] = v
                
                # Nạp state_dict đã sửa vào mô hình
                facerestore_model_obj.load_state_dict(new_state_dict, strict=True)
                facerestore_model_obj.eval()
                
            elif "codeformer" in face_restore_model.lower():
                codeformer_net = ARCH_REGISTRY.get("CodeFormer")(
                    dim_embd=512, codebook_size=1024, n_head=8, n_layers=9, connect_list=["32", "64", "128", "256"],
                ).to(device)
                checkpoint = torch.load(model_path)["params_ema"]
                codeformer_net.load_state_dict(checkpoint)
                facerestore_model_obj = codeformer_net.eval()
            elif ".onnx" in face_restore_model:
                facerestore_model_obj = set_ort_session(model_path, providers=providers)
            else:
                sd = comfy.utils.load_torch_file(model_path, safe_load=True)
                facerestore_model_obj = model_loading.load_state_dict(sd).eval()
                facerestore_model_obj.to(device)
            # ------------------------------------------------------

            if faceSize != FACE_SIZE or self.face_helper is None:
                # 1. Khởi tạo "đánh lừa" r_facelib để vượt qua bước check mặc định
                fallback_det = facedetection if facedetection != "scrfd" else "retinaface_resnet50"
                self.face_helper = FaceRestoreHelper(1, face_size=faceSize, crop_ratio=(1, 1), det_model=fallback_det, save_ext='png', use_parse=True, device=device)
                
                # 2. Xài ké file scrfd_2.5g.onnx từ thư mục FaceFusion
                if facedetection == "scrfd":
                    import insightface # Chỉ giữ lại import insightface vì nó chưa có ở global
                    
                    # Trỏ đường dẫn thẳng sang nhà hàng xóm Facefusion
                    ff_models_dir = os.path.join(folder_paths.base_path, "custom_nodes", "Facefusion_comfyui", "models")
                    scrfd_path = os.path.join(ff_models_dir, "scrfd_2.5g.onnx")
                    
                    if os.path.exists(scrfd_path):
                        # Khởi tạo trực tiếp file onnx thay vì gọi qua engine của ReActor
                        det_model = insightface.model_zoo.get_model(scrfd_path, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
                        ...
                        class SCRFDWrapper:
                            ...
                            def detect_faces(self, image, max_num=0):
                                bboxes, kpss = self.model.detect(image, max_num=max_num, metric='default')
                                
                                if bboxes is None or len(bboxes) == 0:
                                    return np.zeros((0, 15)), np.zeros((0, 15)) # Trả về mảng rỗng tương thích
                                
                                # SỬA LỖI Ở ĐÂY:
                                # kpss có shape (N, 5, 2). Chúng ta cần làm phẳng thành (N, 10).
                                # Dùng -1 để tự tính toán kích thước còn lại thay vì truyền tuple vào
                                kpss_flattened = kpss.reshape(kpss.shape[0], -1)
                                
                                # Ghép bboxes (N, 5) và kpss_flattened (N, 10) thành (N, 15)
                                dets = np.concatenate((bboxes, kpss_flattened), axis=1)
                                return dets

                        # Ép lõi phục hồi xài engine vừa tạo
                        self.face_helper.face_det = SCRFDWrapper(det_model)
                    else:
                        print(f"[ReActor] LỖI: Không tìm thấy {scrfd_path}. Đang dùng tạm RetinaFace.")
            ...
            for i in range(total_images):
                ...
                if facerestore_model_obj is None or self.face_helper is None:
                    return result
                    ...
                    if swapped_bboxes and self.restore_swapped_only:
                        ...
                        if current_bbox is not None:
                            for s_idx, sbox in enumerate(swapped_bboxes):
                                if used_swapped is not None and used_swapped[s_idx]: continue
                                if _iou(current_bbox, sbox) >= IOU_THRESHOLD:
                                    cx1, cy1 = (current_bbox[0]+current_bbox[2])/2, (current_bbox[1]+current_bbox[3])/2
                                    cx2, cy2 = (sbox[0]+sbox[2])/2, (sbox[1]+sbox[3])/2
                                    if abs(cx1 - cx2) < (current_bbox[2]-current_bbox[0])*0.25 and abs(cy1 - cy2) < (current_bbox[3]-current_bbox[1])*0.25:
                                        do_restore = True
                                        if used_swapped is not None: used_swapped[s_idx] = True
                                    break
                        ...
                        try:
                            with torch.no_grad():
                                if ".onnx" in face_restore_model:
                                    ort_session = facerestore_model_obj
                                    ort_session_inputs = {}
                                    for ort_session_input in ort_session.get_inputs():
                                        if ort_session_input.name == "input": ort_session_inputs[ort_session_input.name] = prepare_cropped_face(cropped_face)
                                        if ort_session_input.name == "weight": ort_session_inputs[ort_session_input.name] = np.array([1], dtype=np.double)
                                    output = ort_session.run(None, ort_session_inputs)[0][0]
                                    restored_face = normalize_cropped_face(output)
                                else:
                                    output = facerestore_model_obj(cropped_face_t, w=codeformer_weight)[0] if "codeformer" in face_restore_model.lower() else facerestore_model_obj(cropped_face_t)[0]
            ...
            result = torch.from_numpy(restored_img_np)
...
class RestoreFaceAdvanced:
    ...
    def execute(
        ...
        if face_restore_model != "none" and not model_management.processing_interrupted():
            ...
            if faceSize != FACE_SIZE or self.face_helper is None:
                # Nếu user chọn scrfd, truyền tạm retinaface để lách lỗi của r_facelib
                fallback_det = facedetection if facedetection != "scrfd" else "retinaface_resnet50"
                self.face_helper = FaceRestoreHelper(1, face_size=faceSize, crop_ratio=(1, 1), det_model=fallback_det, save_ext='png', use_parse=True, device=device)
                
                # Cầu nối tiêm SCRFD vào FaceRestoreHelper
                if facedetection == "scrfd":
                    from scripts.reactor_swapper import getAnalysisModel
                    # Khởi tạo mô hình phân tích gốc chứa SCRFD 10G
                    analyzer = getAnalysisModel()
                    ...
                    class SCRFDWrapper:
                        ...
                        def detect_faces(self, image, max_num=0):
                            # Lấy bbox và điểm neo (kps) từ SCRFD
                            bboxes, kpss = self.det_model.detect(image, max_num=max_num, metric='default')
                            return bboxes, kpss

                    # Ghi đè bộ dò tìm (Detector) mặc định bằng siêu nhận diện SCRFD
                    self.face_helper.face_det = SCRFDWrapper(analyzer.models["det"])
